...
class OpenAIWebSearchTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        if env.api_mode != "responses" or not env.enable_web_search:
            return (
                "已跳过联网搜索：当前 API 配置未启用 Responses API web_search。"
                "如果你的服务商支持 Responses 工具调用，请在侧边栏把后端直连接口改为 responses，"
                "并勾选联网搜索；否则请使用文献上传或在主题中提供资料来源。"
            )

        try:
            base_url = validate_base_url(env.base_url)
        except ValueError as exc:
            return f"搜索失败：Base URL 安全校验未通过：{exc}"
        ...
        for tool_type in ("web_search", "web_search_preview"):
            ...
            request = urllib.request.Request(
                ...
                headers={
                    ...
                    "User-Agent": "auto-report-agent/0.2",
                },
            )
            ...
            try:
                ...
                last_error = f"HTTP {exc.code}，响应：{body[:1000]}"
                if tool_type == "web_search" and exc.code in {400, 404, 422}:
                    continue
        ...
        if data is None:
            return f"搜索失败：新旧 Web Search 工具均不可用。{last_error}"
